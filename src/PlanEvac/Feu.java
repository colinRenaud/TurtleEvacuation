...
public class Feu extends Turtle{
    ...
    private final int timeBound = 500; // each timeBound speed there is a fire generation
    ...
    private final int distanceWithOld = 5; // distance between current and new fire generated 
	...
	protected String act(){
		...
		if(dureePresence % timeBound == 0)  // each time bound speed there is fire generation
			sePropager();
		return "act";	
	}
	
	
	protected void sePropager() {
		if(nbPropagation < nbPropagationMAX + 1) {
			System.out.println("le feu se propage !!!");
			Feu f1 = new Feu();
			f1.propagé = true;
			nbPropagation++;
			//  generated fire is moved according to the angus with the generator fire
			f1.newX = (int) (xcor() + distanceWithOld * nbPropagation * +Math.cos(Math.toRadians(propagationAngle))); 
			f1.newY = (int) (ycor() + distanceWithOld * nbPropagation * +Math.sin(Math.toRadians(propagationAngle)));	
